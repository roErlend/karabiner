...
export function openLocal(...what: string[]): LayerCommand {
  return {
    to: what.map((w) => ({
      shell_command: `open ${w}`,
    })),
    description: `Open locally ${what.join(" & ")}`,
  };
}
 ...
 */
export function app(name: string, local: boolean = false): LayerCommand {
  const cmd = `-a '${name}.app'`;
  return local ? openLocal(cmd) : open(cmd);
}
